...
struct AttackTransitionJob : IJobForEachWithEntity<State, Translation, TargetPos, AttackRange>
{
    public void Execute(Entity entity, int index, ref State c0, [ReadOnly] ref Translation c1, [ReadOnly] ref TargetPos c2, [ReadOnly] ref AttackRange c3)
    {
        if(TransitionRules.TransitionToAttack(c3, c1, c2.Value))
        {
            if(c0.Value != States.Attack)
            {
                RemoveStateData(c0.Value, entity, index);
                TransitionSystem.buffer.AddComponent(index, entity, UnitData.DefaultAttackData);
